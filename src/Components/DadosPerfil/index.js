  ...
  <Fragment>
    <Container>
      <Section>
        <h2>PERFIL</h2>
        {usuario.avatar ? (
          <Link to={`/avataruser/${usuario._id}/alterar`}>
            <img src={usuario.avatar.url} alt="Avatar do Usuario" />
          </Link>
        ) : (
          <Link to={`/avataruser/${usuario._id}/cadastro`}>
            {' '}
            <img
        ...
        <p>
          Criado em{' '}
          {format(usuario && usuario.createdAt, 'DD/MM/YYYY', { locale: pt })}{' '}
        </p>
        ...
        <footer>
          <Link to={`/usuarios/${usuario._id}/alterar`}>
            <button className="editar">Editar</button>
          </Link>
          <button className="excluir" onClick={() => deletarConta(usuario._id)}>
            Excluir
          </button>
        </footer>
      </Section>
      <Depoimento>
        ...
        <p>
          "
          {depoimento && depoimento.descricao
        ...
        <footer>
          {depoimento.descricao ? (
            <Fragment>
              <Link to={`/depoimentos/alterar/${depoimento._id}`}>
                <button className="editar">Editar</button>
              </Link>
              <button
                className="excluir"
                onClick={() => deletarDepo(depoimento._id)}
              >
                Excluir
              </button>
            </Fragment>
          ) : (
            <Link to="/depoimentos/criar">
